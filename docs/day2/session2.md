...
VM은 부팅에 수 분이 걸리지만, 컨테이너는 **프로세스를 실행하는 것**이라 수 초 만에 시작됩니다.

| | VM | 컨테이너 |
|---|---|---|
| 실체 | 독립된 운영체제 | 호스트 OS 위에서 실행되는 **프로세스** |
| 시작 | OS 부팅 (수 분) | 프로세스 실행 (수 초) |
| 종료 | OS 셧다운 | 프로세스 종료 |

그리고 프로세스는 **실행 시 환경변수를 받을 수 있습니다.**
같은 이미지라도 어떤 환경변수를 주느냐에 따라 **동작이 달라집니다.**

---

## STEP 01 — 환경변수로 달라지는 컨테이너 실습

> 같은 이미지, 환경변수만 다르게 → 화면이 달라집니다.

### ① 파란 화면 컨테이너 실행

```bash
docker run -d -p 9090:8080 -e APP_COLOR=blue --name app-blue kodekloud/webapp-color
```

### ② 초록 화면 컨테이너 실행

```bash
docker run -d -p 9091:8080 -e APP_COLOR=green --name app-green kodekloud/webapp-color
```
...
xxxxxxxxxxxx   kodekloud/webapp-color     0.0.0.0:9091->8080/tcp   app-green
xxxxxxxxxxxx   kodekloud/webapp-color     0.0.0.0:9090->8080/tcp   app-blue
```

IMAGE가 완전히 동일합니다. 환경변수(`-e`)만 달랐습니다.

### ④ 브라우저에서 확인

```
http://<App-VM-플로팅-IP>:9090  → 파란 화면
http://<App-VM-플로팅-IP>:9091  → 초록 화면
```

!!! tip "이게 바로 환경변수 주입입니다"
    `-e APP_COLOR=blue` 처럼 `-e 키=값` 형태로 컨테이너 실행 시 설정을 주입합니다.
    이미지는 그대로이고, 동작만 달라집니다.
    DB 주소, 비밀번호, 서비스 URL 등 **환경마다 다른 값은 모두 이 방식으로 주입**합니다.
...
1일차 실습 후 인스턴스를 종료했다면 **DB VM을 먼저 켜야** 합니다.
컨테이너 실행 전에 DB가 떠 있지 않으면 앱이 DB에 연결하지 못합니다.

```
Compute > Instance > minwon-db-01 선택 > 인스턴스 시작
```

상태가 **`ACTIVE`** 로 바뀔 때까지 기다립니다.

!!! warning "DB가 켜지기 전에 컨테이너를 실행하면?"
    앱이 시작되자마자 DB 연결 오류가 발생합니다.
    반드시 DB VM이 `ACTIVE` 상태가 된 후 다음 단계로 진행하세요.
...
| # | 확인 항목 | 확인 방법 |
|---|---------|---------|
| ① | 같은 이미지로 환경변수만 다르게 해서 두 컨테이너를 실행했다 | 브라우저에서 색 확인 |
...
지금까지 컨테이너를 직접 실행하고 관리해봤습니다.
잘 동작하지만, 한 가지씩 생각해봅시다.

| 상황 | 지금 방식으로는? |
|------|----------------|
| 컨테이너가 갑자기 죽었다 | 직접 SSH 접속해서 다시 `docker run` |
| 트래픽이 갑자기 몰린다 | 직접 SSH 접속해서 컨테이너 더 띄우기 |
| 새 버전으로 교체하고 싶다 | `docker stop` → `docker rm` → `docker run` 수동 반복 |
| 서버가 10대라면? | 10대 모두 접속해서 같은 작업 반복 |
| 컨테이너가 응답을 안 한다 | 사람이 직접 확인하고 재시작 |
...
!!! warning "이건 운영이 아니라 노동입니다"
    컨테이너 하나를 손으로 띄우는 것과,
    수십 개의 컨테이너를 안정적으로 **자동으로** 운영하는 것은 완전히 다른 문제입니다.

**자동으로 재시작해줄 수 없을까?**
**알아서 여러 대에 나눠서 실행해줄 수 없을까?**
**새 버전을 서비스 중단 없이 바꿔줄 수 없을까?**

이 질문들에 답하는 것이 **Kubernetes** 입니다.
...
![NKS 기본 노드 그룹 설정 — 이미지·인스턴스 타입·블록 스토리지 변경](./images/3-1-nks-node-group-settings.png)
> 📌 위 화면은 참고용입니다. 실제 화면 구성이 다를 수 있으니 **텍스트 지시를 기준으로** 진행하세요.
