...
두 멤버(`minwon-app-01`, `minwon-app-02`)가 모두 **ACTIVE** 상태이면 부하 분산이 정상 동작하는 것입니다.
...
### 5-2. 브라우저 헬스체크 메뉴로 부하 분산 확인

민원 서비스 상단의 **헬스체크** 메뉴를 클릭하면 현재 응답한 서버의 IP 주소를 확인할 수 있습니다.
페이지를 새로고침할 때마다 IP 주소가 `minwon-app-01`과 `minwon-app-02`로 번갈아 바뀌는 것을 확인할 수 있습니다.

![헬스체크 메뉴 — 서버 IP 확인 1](./images/5-5-health-check-1.png)
> 📌 위 화면은 참고용입니다. 실제 화면 구성이 다를 수 있으니 **텍스트 지시를 기준으로** 진행하세요.
