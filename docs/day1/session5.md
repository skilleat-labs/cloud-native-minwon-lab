...
같은 방법으로 `minwon-app-02` 도 멤버로 추가합니다.
...
헬스체크가 통과되었다면 이제 **LB에 연결된 공인 IP**로 서비스에 접속할 수 있습니다.

이전에는 App VM의 플로팅 IP와 `:8080` 포트로 직접 접속했지만, 이제는 LB의 공인 IP로 접속하면 됩니다. 포트 번호 없이 접속되는 것을 확인할 수 있습니다.

```
http://<LB-공인-IP>
```

![LB 공인 IP로 서비스 접속 확인](./images/5-4-lb-public-ip-access.png)
> 📌 위 화면은 참고용입니다. 실제 화면 구성이 다를 수 있으니 **텍스트 지시를 기준으로** 진행하세요.

---

## STEP 05 — 부하 분산 확인 및 서비스 접속
...
```bash
for i in 1 2 3 4 5 6; do curl -s http://<LB-플로팅-IP>/health; echo; done
```

아래와 같이 두 서버가 번갈아 응답하면 LB 분산이 정상 동작하는 것입니다.

```json
{"server":"192.168.0.10","status":"ok"}
{"server":"192.168.0.62","status":"ok"}
{"server":"192.168.0.10","status":"ok"}
{"server":"192.168.0.62","status":"ok"}
```

!!! info "브라우저 새로고침으로 확인이 안 되는 이유"
    Chrome, Edge, Firefox, Safari 등 **모든 브라우저**는 성능을 위해 HTTP Keep-Alive를 사용합니다.
    한 번 맺은 TCP 연결을 재사용하기 때문에 새로고침해도 같은 서버로 계속 연결됩니다.
    브라우저 종류와 상관없이 동일한 현상이며, 이는 LB 오류가 아닙니다.
    **curl은 요청마다 새 연결을 맺기 때문에** 부하 분산을 정확하게 확인할 수 있습니다.
