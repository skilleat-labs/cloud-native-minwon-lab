# Day 1 · 6차시 — 민원 데이터와 첨부파일을 안전하게 저장하라
...
!!! info "Object Storage 서비스 활성화는 2차시에서 이미 완료했습니다"
...
| username | **IAM 계정 ID** (예: `gov4.edu1`) | 강사에게 받은 계정 |
...
!!! info "SSH 접속을 위해 플로팅 IP를 임시로 생성합니다"
    5차시에서 App VM의 플로팅 IP를 LB로 옮겼기 때문에, 지금 App VM에는 공인 IP가 없습니다.
...
!!! warning "터미널에 바로 붙여넣지 마세요 — 메모장에서 먼저 값을 바꾸세요"
    1. 아래 명령어를 **메모장(또는 텍스트 편집기)** 에 붙여넣습니다
    2. `여기에...` 부분을 본인 값으로 수정합니다
    3. 수정이 완료된 전체 명령어를 복사해 터미널에 붙여넣습니다
...
| `여기에IAM계정ID입력` | IAM 계정 ID (예: `gov4.edu1`) |
...
!!! tip "minwon-app-01 작업이 끝났다면 minwon-app-02에도 동일하게 진행하세요"
    3-0 ~ 3-1 과정을 `minwon-app-02`에 대해 똑같이 반복합니다.
    네트워크 인터페이스 선택 시 `minwon-app-02`를 선택하세요.

---

!!! question "생각해보기 — 서버가 10대라면?"
    지금은 서버가 2대라서 각각 SSH로 접속해 설정했습니다.
    만약 실제 서비스에서 App VM이 10대라면 어떨까요?

    우리가 앞서 배웠던 기능 중 **어떤 것을 활용하면 처음부터 이 설정을 자동으로 넣을 수 있을까요?**

    힌트: 인스턴스를 생성할 때 입력했던 항목을 떠올려 보세요.
...
2. **연결 해제** 클릭 → 확인
3. 같은 IP를 다시 체크 후 **플로팅 IP 삭제** 클릭 → 확인

![플로팅 IP 연결 해제 및 삭제](./images/6-3-2-floating-ip-delete.png)
> 📌 위 화면은 참고용입니다. 실제 화면 구성이 다를 수 있으니 **텍스트 지시를 기준으로** 진행하세요.

!!! warning "플로팅 IP는 연결하지 않아도 과금됩니다"
    연결 해제만 하고 삭제하지 않으면 계속 과금됩니다. 반드시 **삭제까지** 완료하세요.

---

## STEP 04 — 첨부파일 업로드 및 확인

### 4-1. 민원 접수 시 첨부파일 업로드
...
오늘 실습이 모두 끝났으면 인스턴스를 **종료** 해두세요.
삭제가 아닌 종료이므로 데이터는 그대로 유지되고, 2일차에 다시 시작할 수 있습니다.

!!! info "NHN Cloud 중지 vs 종료 차이"
    | 구분 | 중지(Stop) | 종료(Shelve) |
    |------|-----------|------------|
    | 하이퍼바이저 리소스 | 할당 유지 | 반납 |
    | 인스턴스 비용 | **중지 요금 발생** | **무료** |
    | 재시작 | 가능 | 가능 |
    ...
    비용 절감을 위해 **종료**를 사용하세요.
    Block Storage 등 연결된 자원은 종료 후에도 계속 과금됩니다.
...
![인스턴스 종료 화면](./images/6-instance-shelve.png)
> 📌 위 화면은 참고용입니다. 실제 화면 구성이 다를 수 있으니 **텍스트 지시를 기준으로** 진행하세요.

!!! warning "종료 ≠ 삭제"
    **종료(Shelve)** 는 인스턴스를 보존한 채 자원만 반납하는 것입니다.
    **삭제(Delete)** 는 인스턴스와 데이터가 모두 사라지므로 절대 혼동하지 마세요.
