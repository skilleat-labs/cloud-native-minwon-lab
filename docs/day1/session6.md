...
오늘 실습이 모두 끝났으면 인스턴스를 **중지(STOP)** 해두세요.
삭제가 아닌 중지이므로 데이터는 그대로 유지되고, 2일차에 다시 시작할 수 있습니다.

!!! info "중지 시 비용 절감 효과"
    인스턴스를 중지하면 **컴퓨팅(vCPU·RAM) 비용이 발생하지 않습니다.**
    Block Storage, Floating IP 등 연결된 자원은 계속 과금되므로 플로팅 IP는 미리 해제·삭제하세요.
...
| 인스턴스 | 중지 방법 |
|---------|---------|
| `minwon-app-01` | 체크 → 인스턴스 중지 클릭 |
| `minwon-app-02` | 체크 → 인스턴스 중지 클릭 |
| `minwon-db-01` | 체크 → 인스턴스 중지 클릭 |

!!! warning "삭제가 아닌 중지입니다"
    **중지(STOP)** 와 **삭제(DELETE)** 를 혼동하지 마세요.
    삭제하면 인스턴스와 데이터가 모두 사라집니다.
