...
#### 서비스가 정상 배포되었는지 브라우저로 확인해봅시다!

공인 IP를 확인했다면, Chrome 또는 Edge를 열고 아래 주소로 접속해보세요.
...
!!! tip "눈으로 먼저 확인!"
    SSH 접속 전에 **브라우저에서 먼저 서비스가 뜨는지 눈으로 확인**합니다.
    페이지가 정상적으로 보인다면 App VM과 사용자 스크립트가 모두 성공적으로 완료된 것입니다.
...
위의 브라우저 화면처럼 민원 서비스 페이지가 정상적으로 보이면 DB 연결이 정상적으로 되고 있는 것입니다.
...
2. 상단 **이미지 생성** 버튼을 클릭합니다
3. 이미지 이름을 입력합니다 (예: `minwon-app-image-01`)
4. 하단 체크박스(파일 시스템 무결성 동의)를 **체크**합니다
5. **확인** 을 클릭합니다

![이미지 생성 다이얼로그](./images/6-7-instance-image-create.png)
> 📌 위 화면은 참고용입니다. 실제 화면 구성이 다를 수 있으니 **텍스트 지시를 기준으로** 진행하세요.
