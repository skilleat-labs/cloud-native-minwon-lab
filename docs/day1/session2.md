...
| `0.0.0.0/0` | 인터넷 게이트웨이 | 외부 인터넷 통신 |

`0.0.0.0/0 → 인터넷 게이트웨이` 경로가 있어야 플로팅 IP를 통해 외부에서 접속할 수 있습니다.

!!! warning "인터넷 게이트웨이 없이 플로팅 IP를 붙여도 외부 접속이 되지 않습니다"
    플로팅 IP는 주소일 뿐입니다. 실제로 인터넷과 연결되는 경로는 인터넷 게이트웨이가 담당합니다.
